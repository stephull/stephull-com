...
export default function BlogTimelineItemCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const blogTimelineItem = await DataStore.save(
            new BlogTimelineItem(modelFieldsToSave)
          );
          const promises = [];
          promises.push(
            ...blogcomments.reduce((promises, original) => {
              promises.push(
                DataStore.save(
                  Comment.copyOf(original, (updated) => {
                    updated.blogtimelineitemID = blogTimelineItem.id;
                  })
                )
              );
              return promises;
          ...
          promises.push(
            ...blogcontents.reduce((promises, original) => {
              promises.push(
                DataStore.save(
      ...
      {...getOverrideProps(overrides, "BlogTimelineItemCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
