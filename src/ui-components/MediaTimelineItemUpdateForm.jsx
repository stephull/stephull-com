...
export default function MediaTimelineItemUpdateForm(props) {
  const {
    id: idProp,
    mediaTimelineItem: mediaTimelineItemModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mediaTimelineItemRecord
      ? {
          ...initialValues,
          ...mediaTimelineItemRecord,
          mediacomments: linkedMediacomments,
          mediacontents: linkedMediacontents,
        }
      : initialValues;
    setDescription(cleanValues.description);
    setCreatedAt(cleanValues.createdAt);
    setStars(cleanValues.stars);
    setMediacomments(cleanValues.mediacomments ?? []);
    setCurrentMediacommentsValue(undefined);
    setCurrentMediacommentsDisplayValue("");
    setMediacontents(cleanValues.mediacontents ?? []);
    setCurrentMediacontentsValue(undefined);
    setCurrentMediacontentsDisplayValue("");
    setDesc(cleanValues.desc);
    setErrors({});
  };
  const [mediaTimelineItemRecord, setMediaTimelineItemRecord] = React.useState(
    mediaTimelineItemModelProp
  );
  const [linkedMediacomments, setLinkedMediacomments] = React.useState([]);
  const canUnlinkMediacomments = true;
  const [linkedMediacontents, setLinkedMediacontents] = React.useState([]);
  const canUnlinkMediacontents = true;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(MediaTimelineItem, idProp)
        : mediaTimelineItemModelProp;
      setMediaTimelineItemRecord(record);
      const linkedMediacomments = record
        ? await record.mediacomments.toArray()
        : [];
      setLinkedMediacomments(linkedMediacomments);
      const linkedMediacontents = record
        ? await record.mediacontents.toArray()
        : [];
      setLinkedMediacontents(linkedMediacontents);
    };
    queryData();
  }, [idProp, mediaTimelineItemModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const mediacommentsToLink = [];
          const mediacommentsToUnLink = [];
          const mediacommentsSet = new Set();
          const linkedMediacommentsSet = new Set();
          mediacomments.forEach((r) =>
            mediacommentsSet.add(getIDValue.mediacomments?.(r))
          );
          linkedMediacomments.forEach((r) =>
            linkedMediacommentsSet.add(getIDValue.mediacomments?.(r))
          );
          linkedMediacomments.forEach((r) => {
            if (!mediacommentsSet.has(getIDValue.mediacomments?.(r))) {
              mediacommentsToUnLink.push(r);
            }
          });
          mediacomments.forEach((r) => {
            if (!linkedMediacommentsSet.has(getIDValue.mediacomments?.(r))) {
              mediacommentsToLink.push(r);
            }
          });
          mediacommentsToUnLink.forEach((original) => {
            if (!canUnlinkMediacomments) {
              throw Error(
                `Comment ${original.id} cannot be unlinked from MediaTimelineItem because mediatimelineitemID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
                Comment.copyOf(original, (updated) => {
                  updated.mediatimelineitemID = null;
                })
              )
            );
          });
          mediacommentsToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                Comment.copyOf(original, (updated) => {
                  updated.mediatimelineitemID = mediaTimelineItemRecord.id;
                })
              )
            );
          });
          const mediacontentsToLink = [];
          const mediacontentsToUnLink = [];
          const mediacontentsSet = new Set();
          const linkedMediacontentsSet = new Set();
          mediacontents.forEach((r) =>
            mediacontentsSet.add(getIDValue.mediacontents?.(r))
          );
          linkedMediacontents.forEach((r) =>
            linkedMediacontentsSet.add(getIDValue.mediacontents?.(r))
          );
          linkedMediacontents.forEach((r) => {
            if (!mediacontentsSet.has(getIDValue.mediacontents?.(r))) {
              mediacontentsToUnLink.push(r);
            }
          });
          mediacontents.forEach((r) => {
            if (!linkedMediacontentsSet.has(getIDValue.mediacontents?.(r))) {
              mediacontentsToLink.push(r);
            }
          });
          mediacontentsToUnLink.forEach((original) => {
            if (!canUnlinkMediacontents) {
              throw Error(
                `Content ${original.id} cannot be unlinked from MediaTimelineItem because mediatimelineitemID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
          ...
          mediacontentsToLink.forEach((original) => {
            promises.push(
              DataStore.save(
                Content.copyOf(original, (updated) => {
                  updated.mediatimelineitemID = mediaTimelineItemRecord.id;
          ...
          promises.push(
            DataStore.save(
              MediaTimelineItem.copyOf(mediaTimelineItemRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MediaTimelineItemUpdateForm")}
      {...rest}
    >
      <TextField
        label="Description"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mediaTimelineItemModelProp)}
